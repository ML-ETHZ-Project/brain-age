...
- Approach: `StackingRegressor` (GradientBoosting + Ridge(α=10) + KNN(k=15, distance) → Ridge meta-learner) on `CorrelationPrunedKBest`-100 features, median impute, no outlier removal (`notebooks/stacking_with_corr_pruned.py`; not yet folded into `src/baseline.py`, see row above)
- CV score: R²=0.5229 (5-fold)
- Public LB: best individual submission 0.6528 (this approach); best blend 0.6730 (50/50 with Philippe's `submission_stack.csv`); best single submission on record 0.6812 (Philippe's tuned stack, `philippe/ensembling` branch, CV R²≈0.5222 — statistically tied with ours, gap likely public-LB noise, see log above)
