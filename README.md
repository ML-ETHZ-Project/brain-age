...
- Approach: Optuna-tuned XGBoost on `TopCorrMixedImputer`(n_fancy=40) + `SelectKBest`(f_regression, k=200) features, no outlier removal (`src/advanced_pipeline.py`) — supersedes the `src/baseline.py` GradientBoosting approach below, which remains in the repo as the simpler reference/fallback.
- CV score: R²=0.5518 ± 0.0225 (5-fold); 90% bootstrap CI (100 resamples, OOB scoring) = [0.4185, 0.5620] (see note above on why the OOB estimate reads lower than the CV number)
- Public LB score: not yet submitted
- Runner-up: plain Optuna-tuned XGBoost on the original baseline preprocessing (median impute, k=100), R²=0.5398 — simpler, nearly as strong, submitted as `submissions/xgboost_tuned_baseline_preproc_submission.csv` as a hedge
- Previous best (kept as reference/fallback): GradientBoostingRegressor on SelectKBest-100 features, median impute, no outlier removal (`src/baseline.py`), R²=0.5065 (5-fold)
